defmodule InvoixWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, InvoixWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    test "erases session and cookies", %{conn: conn, user: user} do
      ...
      conn =
        ...
        |> put_req_cookie(@current_user_cookie, "current_user")
        |> fetch_cookies()
      ...
      assert %{max_age: 0} = conn.resp_cookies[@current_user_cookie]
